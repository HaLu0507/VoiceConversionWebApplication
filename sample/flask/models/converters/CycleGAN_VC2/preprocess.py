...
def melGAN_decompose(wav, n_mel_channels=80):
    """
    MelGAN を用いてメルスペクトログラムを求め, それを返すメソッド

    Args: 
        wav : 音声ファイル
        n_mel_channels : メルスペクトログラムの周波数の次元数

    Returns:
        mel_sp : メルスペクトログラム
    """
    fft = Audio2Mel(n_fft=1024, hop_length=256, win_length=256, n_mel_channels=n_mel_channels)

    wav = 0.95 * normalize(wav)
    wav = torch.from_numpy(wav.astype(np.float32)).clone() # numpy to tensor
    wav = wav.unsqueeze(0)
    mel_sp = fft(wav) # mel_sp (1, hz, time)
    mel_sp = mel_sp.squeeze(0) # mel_sp (hz, time)
    mel_sp = mel_sp.transpose(0,1) # (time, hz)
    mel_sp = mel_sp.to('cpu').detach().numpy().copy() # tensor to numpy

    return mel_sp

def melGAN_encode_data(wave, n_mel_channels=80):
    """
    MelGAN を用いて, 音声をメルスペクトログラムに変換するメソッド

    Args:
        wave: 複数の音声ファイル
        fs  : サンプリング周波数
        frame_period : frame_period [ms] ごとにメルスペクトログラムを求める
        mel_channels : メルスペクトルグラムの周波数の次元数

    Returns:
        mel_sps : メルスペクトルグラム (num_sample_voice, Time, Hz)
    """
    mel_sps = list()
    for wav in wave:
        mel_sp = melGAN_decompose(wav, n_mel_channels)
        mel_sps.append(mel_sp)
    return mel_sps

def librosa_decompose(wav, n_mel_channlels=80):
    """
    librosa を用いてメルスペクトログラムを求め, それを返すメソッド
    ...
    Returns:
        mel_sp : メルスペクトログラム (time, hz)
    """
    clip = 0.001

    mel_sp = librosa.stft(
        wav, n_fft=1024, hop_length=256, win_length=256
    )

    # メルフィルタバンク
    mel_basis = librosa.filters.mel(sr=22050, n_fft=1024, n_mels=n_mel_channlels)
    # スペクトログラム -> メルスペクトログラム
    mel_sp = np.dot(mel_basis, np.abs(mel_sp))
    # クリッピング : ある一定以下の値を0にする
    # mel_sp = np.maximum(mel_sp, clip)
    # 対数を取る
    mel_sp = np.log10(mel_sp)
    return mel_sp.T 

def librosa_encode_data(wave, n_mel_channels=80):
    """
    librosa を用いて, 音声をメルスペクトログラムに変換するメソッド
    ...
    mel_sps = list()
    for wav in wave:
        mel_sp = librosa_decompose(wav, n_mel_channels)
        mel_sps.append(mel_sp)
    return mel_sps
